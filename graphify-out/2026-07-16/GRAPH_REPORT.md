...
## Corpus Check
- 37 files · ~41,720 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 516 nodes · 1341 edges · 44 communities (32 shown, 12 thin omitted)
...
Cohesion: 0.10
Nodes (42): acc, Binder, Binding, Bundle, JMSBinding, LeaderElectionModel, MQBinder, Prop (+34 more)

### Community 1 - "Spec Model & Validation"
Cohesion: 0.16
Nodes (22): Node, applyDest(), Node, nodePtr(), Defaults, LeaderElection, Management, rawDefaults (+14 more)
...
Cohesion: 0.11
Nodes (18): 10. `examples`, 11. Notes and gotchas, 1. Running solmq-gen, 2. Quick start, 3. Commands, 4. The spec folder, 5.1 Top-level, 5.2 `solace:` options (+10 more)
...
Cohesion: 0.11
Nodes (36): checkConnections(), checkDuplicateSources(), checkKeyAliasConflicts(), checkKube(), checkLeaderElection(), checkLibs(), checkSide(), checkSyslog() (+28 more)

### Community 22 - "solmq-gen — Solace IBM MQ Connector config generator"
Cohesion: 0.18
Nodes (10): Build, `defaults.yaml` (optional, committed — no secret values), Design notes, File model, `kubernetes.yaml` (deploy only; `-k` overrides), Release (CI), solmq-gen — Solace IBM MQ Connector config generator, Usage (+2 more)
...
### Community 43 - "consolidate_test.go"
Cohesion: 0.41
Nodes (13): binderNames(), binderOf(), eqStrs(), Model, T, mqSide(), solaceSide(), TestBinderDedupAcrossWorkflows() (+5 more)

## Knowledge Gaps
- **81 isolated node(s):** `github.com/solacecommunity/hafio-solace/connectors/ibmmq/solmq-gen`, `Model`, `graphify`, `Build`, `Usage` (+76 more)
...
- **Why does `Build()` connect `Binding Consolidation` to `Spec Model & Validation`, `Generation Orchestration`, `application.yml Rendering`, `consolidate_test.go`, `TLS Store Paths`, `consolidate_test.go`?**
  _High betweenness centrality (0.144) - this node is a cross-community bridge._
- **Why does `Defaults` connect `Spec Model & Validation` to `Binding Consolidation`, `Validation Test Suite`, `Kubernetes Deploy Rendering`, `Generation Orchestration`, `Spec Parsing Tests`, `TLS Store Paths`, `consolidate_test.go`?**
