...
complete -c solmq-conn-util -n '__fish_use_subcommand' -a 'logs' -d 'Print each instance log, where status says what but not why'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from status sts' -l all -d 'reach every connector instance found by image name (solace-pubsub-connector-ibmmq) instead of the ones env.yaml describes -- every namespace on kubernetes, every container on docker/podman; cannot be combined with --pod/--container, nor with --follow under logs'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from status sts' -l pod -r -d 'limit to this kubernetes pod name; repeatable (default: every running pod); no effect on docker/podman'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from status sts' -l container -r -d 'limit to this docker/podman container name; repeatable (default: every running container); no effect on kubernetes'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l all -d 'reach every connector instance found by image name (solace-pubsub-connector-ibmmq) instead of the ones env.yaml describes -- every namespace on kubernetes, every container on docker/podman; cannot be combined with --pod/--container, nor with --follow under logs'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l pod -r -d 'limit to this kubernetes pod name; repeatable (default: every running pod); no effect on docker/podman'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l container -r -d 'limit to this docker/podman container name; repeatable (default: every running container); no effect on kubernetes'
