...
// out of the way of anything that lists the mount directory.
const Filename = ".status-script"

// ContainerDir is the directory the script is written to inside the container.
//
// It is deliberately NOT the libs mount: the script used to live at
// /app/external/libs/status, nested inside a directory the operator can mount a
// whole jar folder onto, which made the two mounts conflict -- a directory
// mount shadows anything nested under it unless the nested mount is declared
// afterwards, so every renderer had to order them just so. As a sibling of
// libs/, spring/ and classpath/ there is nothing to order.
const ContainerDir = "/app/external"
...
# leader-election mode and state, health, and per-workflow state, plus the
# enrichment the CLI's details level shows (uptime, build version, java
# version, the config it read, and heap use), by querying the instance's own
# Spring actuator endpoints from inside the container.
#
# Run it with:
...
# The JVM this instance actually runs on -- the one fact in the report that
# comes from the container rather than from an actuator endpoint. java writes
# -version to stderr, hence the redirect. The first line reads
# 'openjdk version "17.0.9" 2023-10-17'; it is folded to 'openjdk 17.0.9' when
...
if command -v java >/dev/null 2>&1; then
  ...
  if [ -n "${JAVA_LINE:-}" ]; then
    echo "java: $JAVA_LINE"
  elif [ -n "$JAVA_RAW" ]; then
    echo "java: $JAVA_RAW"
  fi
fi

# Which configuration this report was built from: the same search order the
# password lookup above used, first hit first. Usually one path; more than one
# means the connector's configuration is split and the first is the one whose
# values won.
if [ -n "$CONFIG_LIST" ]; then
  echo "config: $CONFIG_LIST"
fi

# Heap use, in raw bytes exactly as Micrometer reported them -- the CLI renders
# them as Mi/Gi. Deliberately not converted here: Jackson may serialise a large
# double in scientific notation (4.32013312E8), which busybox integer
# arithmetic would silently read as 4, while the CLI's own parser handles both
# spellings. A manual reader of this script therefore sees bytes.
#
# The area:heap tag excludes non-heap pools (metaspace, code cache), so the
# number is comparable with -Xmx. An unbounded heap reports a negative maximum,
# which is left out rather than printed as a ceiling.
