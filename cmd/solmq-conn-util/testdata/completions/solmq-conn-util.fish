...
complete -c solmq-conn-util -n '__fish_use_subcommand' -a 'logs' -d 'Print one instance log, where status says what but not why'
complete -c solmq-conn-util -n '__fish_use_subcommand' -a 'cli' -d 'Open a shell inside one instance, or run one command in it'
...
complete -c solmq-conn-util -n '__fish_use_subcommand' -a 'help' -d 'Print this summary, or the help page of one verb'
...
# and offers nothing.
complete -c solmq-conn-util -n '__fish_seen_subcommand_from generate gen' -l platform -r -d 'the platform: kubernetes, docker, or podman (short: kube, dk, pm; default: resolved from env.yaml, or an interactive menu -- see [Platform resolution](commands.md#platform-resolution))'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from deploy dp' -l platform -r -d 'the platform: kubernetes, docker, or podman (short: kube, dk, pm; default: resolved from env.yaml, or an interactive menu -- see [Platform resolution](commands.md#platform-resolution))'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from remove rm' -l no-prompt -d 'tear down without asking anything -- what a script or CI job passes, since the prompts refuse to read a non-TTY rather than hang. It covers both questions: the teardown confirmation, and whether to remove a namespace that turned out to be empty. It cannot authorise more than that: a namespace holding anything this release does not own is never removed, with or without it'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from remove rm' -l platform -r -d 'the platform: kubernetes, docker, or podman (short: kube, dk, pm; default: resolved from env.yaml, or an interactive menu -- see [Platform resolution](commands.md#platform-resolution))'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from status sts' -s v -l verbose -d 'print one line per collection step, naming the call and how long it took, instead of the spinner a terminal gets by default. Both go to stderr, never to stdout, so the report stays byte-identical either way; the step lines are printed whether or not stderr is a terminal, which is what makes 2>steps.log useful for finding which call is slow. Nothing is printed under --watch'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from status sts' -l all -d 'reach every connector instance found by image name (solace-pubsub-connector-ibmmq) instead of the ones env.yaml describes -- every namespace on kubernetes, every container on docker/podman; cannot be combined with --pod/--container'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from status sts' -l platform -r -d 'the platform: kubernetes, docker, or podman (short: kube, dk, pm; default: resolved from env.yaml, or an interactive menu -- see [Platform resolution](commands.md#platform-resolution))'
...
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l follow -d 'keep the log open and print new lines as they arrive, until interrupted (Ctrl-C); reads one instance, so it cannot be combined with --all or --previous'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l previous -d 'read the log of the previous container instead of the running one -- what a pod that is restarting printed before it died; kubernetes only, since neither docker nor podman keeps a prior run under the same name'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l tail -r -d 'read only the last N lines, or all for the whole log (default: all)'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l since -r -d 'read only lines newer than this duration, spelled as a Go duration (30s, 10m, 2h)'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l timestamps -d 'prefix every line with the time the platform recorded for it'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l platform -r -d 'the platform: kubernetes, docker, or podman (short: kube, dk, pm; default: resolved from env.yaml, or an interactive menu -- see [Platform resolution](commands.md#platform-resolution))'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -s e -l env -r -F -d 'config file, relative or absolute path (default: env.yaml)'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l pod -r -d 'the kubernetes pod to reach, by name or by index into the listed order (alphabetical, the order status prints); a name always wins over the index reading. Repeatable on status; on logs and cli, which reach one instance, it may be given once. Default: every running pod on status, and on logs/cli the matching instances are listed instead. No effect on docker/podman'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l container -r -d 'the docker/podman container to reach, by name or by index into the listed order (alphabetical, the order status prints); a name always wins over the index reading. Repeatable on status; on logs and cli, which reach one instance, it may be given once. Default: every running container on status, and on logs/cli the one the section in env.yaml names. No effect on kubernetes'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l namespace -r -d 'kubernetes namespace to query (default: the namespace of the deployment in env.yaml); no effect on docker/podman'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l command -r -d 'override the platform CLI binary (kubectl/oc, docker, or podman) used to reach each instance, instead of the command: in that section'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from logs lg' -l allow-command -r -d 'approve an extra command binary beyond the command: allowlist; repeatable'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from cli' -l platform -r -d 'the platform: kubernetes, docker, or podman (short: kube, dk, pm; default: resolved from env.yaml, or an interactive menu -- see [Platform resolution](commands.md#platform-resolution))'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from cli' -s e -l env -r -F -d 'config file, relative or absolute path (default: env.yaml)'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from cli' -l pod -r -d 'the kubernetes pod to reach, by name or by index into the listed order (alphabetical, the order status prints); a name always wins over the index reading. Repeatable on status; on logs and cli, which reach one instance, it may be given once. Default: every running pod on status, and on logs/cli the matching instances are listed instead. No effect on docker/podman'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from cli' -l container -r -d 'the docker/podman container to reach, by name or by index into the listed order (alphabetical, the order status prints); a name always wins over the index reading. Repeatable on status; on logs and cli, which reach one instance, it may be given once. Default: every running container on status, and on logs/cli the one the section in env.yaml names. No effect on kubernetes'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from cli' -l namespace -r -d 'kubernetes namespace to query (default: the namespace of the deployment in env.yaml); no effect on docker/podman'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from cli' -l command -r -d 'override the platform CLI binary (kubectl/oc, docker, or podman) used to reach each instance, instead of the command: in that section'
complete -c solmq-conn-util -n '__fish_seen_subcommand_from cli' -l allow-command -r -d 'approve an extra command binary beyond the command: allowlist; repeatable'
